...
bot.start(async (ctx) => {
  ...
  try {
    const response = await fetch(
      `http://localhost:3000/users?chat_id=${chatId}`
    );
    ...
    if (data.length > 0) {
      ...
      return ctx.reply(
        "Admin fayl yuklashini kuting. Sizga ma'lumotlaringiz yuboriladi."
      );
    }
...
bot.on("contact", async (ctx) => {
  ...
  if (!contact || !contact.phone_number) {
    return ctx.reply(
      "Telefon raqamni yuborishda xatolik yuz berdi. Iltimos, qaytadan urinib ko'ring."
    );
  }
  ...
  try {
    ...
    if (userData.length === 0) {
      ...
      registeredUsers.add(chatId);
      ctx.reply(
        "Ro'yxatdan muvaffaqiyatli o'tdingiz! Admin fayl yuklaganda sizga ma'lumot yuboriladi."
      );
    } else {
  ...
  try {
    ...
    for (const row of data) {
      ...
      const checkRes = await fetch(
        `http://localhost:3000/data?user_full_name=${encodeURIComponent(
          name
        )}&month=${encodeURIComponent(
          month
        )}&organization_name=${encodeURIComponent(org)}`
      );
    ...
    for (const user of allUsers) {
      ...
      if (userRows.length > 0) {
        ...
        if (userRows.length > 0) {
          const user = userRows[0]; // faqat 1 foydalanuvchini yuboradi, agar ko'p bo'lsa loop qilamiz
          const message = `<b>👤 F.I.Sh:</b> ${user.user_full_name} \n <b>🏢 Tashkilot:</b> ${user.organization_name}\n<b>📆 Oy:</b> ${user["month "]}\n<b>💰 Taxminiy oylik:</b> ${user.estimated_monthly_salary.toLocaleString()} so'm \n<b>📉 Usslab qolish:</b> ${user.withholding.toLocaleString()} so'm \n<b>🧾 To'lanadigan oylik:</b> ${user.monthly_salary.toLocaleString()} so'm \n<b>📱 Tel:</b> +${user.phone_number}\n <b>📝 Izoh:</b> ${user.description === "." || "Izoh berilmagan"}`.trim();
    ...
    if (fullJson.length > 4000) {
      ...
      await ctx.telegram.sendMessage(senderId, `<pre>${fullJson}</pre>`, {
        parse_mode: "HTML",
      });
    }

    await ctx.reply(
      "✅ Barcha foydalanuvchilarga ma'lumot yuborildi va fayl saqlandi."
    );
  } catch (err) {
...
bot.command("users", async (ctx) => {
  ...
  const list = users
    .map((u) => `\ud83d\udcf1 ${u.first_name} - ${u.phone_number} (${u.role})`)
    .join("\n");
